...
public class CubicBezier {
   ...
   * @param bs3 Fourth Bezier Spline Control Point
   * @param eps Maximum angle between line segments
   * @param rec Recursion depth to go
   */
  public CubicBezier(Vector2 p0, Vector2 p1, Vector2 p2, Vector2 p3, float eps, int rec) {
    curvePoints = new ArrayList<Vector2>();
    curveTangents = new ArrayList<Vector2>();
    curveNormals = new ArrayList<Vector2>();
    epsilon = eps;
  ...
  /* de Casteljau points */
  private Vector2 deC(float u, int k, int i){
    if(k == 0){
      switch(i){
        case 0: return p0.clone();
        case 1: return p1.clone();
        case 2: return p2.clone();
        case 3: return p3.clone();
      }
    }
    return deC(u, k-1, i).mul(1.0f - u).add(deC(u, k-1, i+1).mul(u));
  }

  private Vector2 tangentAt(float u){
    return deC(u,2,1).sub(deC(u,2,0)).mul(3);
  }
   ...
   */
  private void tessellate(int rec) {
    if(p1.clone().sub(p0).angle(p2.clone().sub(p1)) < epsilon && p2.clone().sub(p1).angle(p3.clone().sub(p2)) < epsilon || rec <= 0){
      curvePoints.add(p0);
      Vector2 tan = p1.clone().sub(p0).normalize();
      curveTangents.add(tan);
      curveNormals.add(tan.clone().cross().cross().cross().normalize());
    }
    else{
      CubicBezier left = new CubicBezier(p0, deC(0.5f, 1, 0), deC(0.5f, 2, 0), deC(0.5f, 3, 0), epsilon, rec-1);
      CubicBezier right = new CubicBezier(deC(0.5f, 3, 0), deC(0.5f, 2, 1), deC(0.5f, 1, 2), p3, epsilon, rec-1);
      curvePoints.addAll(left.getPoints());
      curvePoints.addAll(right.getPoints());
      curveTangents.addAll(left.getTangents());
      curveTangents.addAll(right.getTangents());
      curveNormals.addAll(left.getNormals());
      curveNormals.addAll(right.getNormals());
    }
  }
