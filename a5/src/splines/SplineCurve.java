...
public abstract class SplineCurve {
  ...
  private void setBeziers() {
    ...
    for(int i=0; i < n - (isClosed ? 0 : 3); i++){
      bezierCurves.add(toBezier(controlPoints.get(i).clone(), controlPoints.get((i+1)%n).clone(), controlPoints.get((i+2)%n).clone(), controlPoints.get((i+3)%n).clone(), epsilon));
    }
  }
  ...
  public static void build3DRevolution(SplineCurve crossSection, OBJMesh mesh, float scale, float sliceTolerance) {
    // TODO A5
    ArrayList<Vector2> scaledpoints = new ArrayList<Vector2>();
    Vector2 uv = new Vector2(0, 0);
    mesh.uvs.add(uv);

    for (int i = 0; i < crossSection.getPoints().size(); i++) {
      Vector2 point = crossSection.getPoints().get(i).clone().mul(scale);
      scaledpoints.add(point);
    }
    if (crossSection.isClosed == true) {
      scaledpoints.add(scaledpoints.get(0));
    }
    int times = (int) Math.ceil((2 * Math.PI / sliceTolerance));

    for (int i = 0; i < scaledpoints.size(); i++) {
      for (int j = 0; j < times; j++) {
        Matrix4 rot = Matrix4.createRotationZ((float) -Math.PI * 2 * j / times);
        Vector3 v = new Vector3(scaledpoints.get(i).x, 0, scaledpoints.get(i).y);
        mesh.positions.add(rot.mulPos(v));
        if (i == scaledpoints.size() - 1) {
          Vector3 n = new Vector3(crossSection.getNormals().get(0).x, 0, crossSection.getNormals().get(0).y);
          mesh.normals.add(rot.clone().transpose().invert().mulDir(n).normalize());
        } else {
          Vector3 n = new Vector3(crossSection.getNormals().get(i).x, 0, crossSection.getNormals().get(i).y);
          mesh.normals.add(rot.clone().transpose().invert().mulDir(n).normalize());
        }
      }
    }
    ...
    for (int i = 0; i < scaledpoints.size() - 1; i++) {
      for (int j = 0; j < times; j++) {
        ...
        face.setVertex(0, (i * times) + (j % times), 0, (i * times) + (j % times));
        face.setVertex(1, (i * times) + (j % times) + times, 0, (i * times) + (j % times) + times);
        face.setVertex(2, (i * times) + ((j + 1) % times) + times, 0, (i * times) + ((j + 1) % times) + times);
        mesh.faces.add(face);
      }
    }
    ...
    for (int i = 0; i < scaledpoints.size() - 1; i++) {
      for (int j = 0; j < times; j++) {
        OBJFace face = new OBJFace(3, true, true);
        face.setVertex(0, (i * times) + (j % times), 0, (i * times) + (j % times));
        face.setVertex(1, (i * times) + ((j + 1) % times) + times, 0, (i * times) + ((j + 1) % times) + times);
        face.setVertex(2, (i * times) + ((j + 1) % times), 0, (i * times) + ((j + 1) % times));
        mesh.faces.add(face);
      }
    }
    try {
