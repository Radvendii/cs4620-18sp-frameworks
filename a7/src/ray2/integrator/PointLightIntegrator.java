...
/**
 * An Integrator that computes a result like the one in Ray 1, which
 * deterministically accounts for only point lights.
 * 
 * @author srm
 */
public class PointLightIntegrator extends Integrator {
	...
	/*
	 * The illumination algorithm is: for each point light in the scene: compute the
	 * light direction and distance evaluate the BRDF add a contribution to the
	 * reflected radiance due to that source
	 * 
	...
	public void shade(Colord outRadiance, Scene scene, Ray ray, IntersectionRecord iRec, int depth) {
		...
		for (Light light : scene.getLights()) {
			if (light instanceof PointLight) {
				...
				if (isShadowed(scene, iRec.location, pointlight.position) == false) {
					Vector3d direction = pointlight.position.clone().sub(iRec.location);
					double r = direction.lenSq();
					Vector3d l = direction.clone().normalize();
					BSDF object = surface.getBSDF();
					object.eval(l, v, normal, c);
					Vector3d I = pointlight.getIntensity();
					double dot = l.dot(normal);
					if (dot < 0) {
						continue;
					}

					color.set(I.clone().mul(c).clone().mul(dot).clone().div(r));
					outRadiance.add(color);
				}
			}
		}
	}
	 ...
	 * intersection point and the given light.
	 * 
	 * @param scene
